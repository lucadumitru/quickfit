(() => {
        ...
        if (wrapper && !isMobile.any()) {
            ...
            function mouseActions(e) {
                if (e.type === "mouseout") cursor.style.opacity = 0; else if (e.type === "mousemove") {
                    ...
                    if (e.target.classList.contains("arrow-left")) cursor.classList.add("_arrow-left"); else cursor.classList.remove("_arrow-left");
                    if (e.target.classList.contains("arrow-right")) cursor.classList.add("_arrow-right"); else cursor.classList.remove("_arrow-right");
    ...
    const heroCanvas = document.querySelector(".hero-section__canvas");
    const heroSection = document.querySelector(".hero-section");
    document.querySelector(".dashboard-section__canvas");
    const observer1 = new MutationObserver((mutationsList => {
        for (const mutation of mutationsList) if (mutation.type === "attributes" && mutation.attributeName === "class") if (footer.classList.contains("_watcher-view")) {
            const footerAnimation = new Application(footerCanvas);
            footerAnimation.load("https://prod.spline.design/Vz-5Vt6JIZf88hZk/scene.splinecode");
            footerAnimation.play();
            observer1.disconnect();
        }
    }));
    const observer2 = new MutationObserver((mutationsList => {
        for (const mutation of mutationsList) if (mutation.type === "attributes" && mutation.attributeName === "class") if (heroSection.classList.contains("_watcher-view")) {
            const heroAnimation = new Application(heroCanvas);
            heroAnimation.load("https://prod.spline.design/Vz-5Vt6JIZf88hZk/scene.splinecode");
            heroAnimation.play();
            observer2.disconnect();
        }
    }));
    const swiperBtnPrev = document.querySelector(".swiper-button-pr");
    const swiperBtnNext = document.querySelector(".swiper-button-nxt");
    const observer3 = new MutationObserver((mutationsList => {
        for (const mutation of mutationsList) if (mutation.type === "attributes" && mutation.attributeName === "class") {
            if (swiperBtnNext.classList.contains("swiper-button-disabled")) swiperBtnNext.removeAttribute("disabled");
            if (swiperBtnPrev.classList.contains("swiper-button-disabled")) swiperBtnPrev.removeAttribute("disabled");
        }
    }));
    observer3.observe(swiperBtnPrev, swiperBtnNext, {
        attributes: true
    });
    const menuList = document.querySelector(".menu__list");
    const menuScrollBtn = document.querySelector(".menu__menu-scroll-btn");
    ...
    function handleResize(entries) {
        for (const entry of entries) {
            const isVisible = isHorizontalScrollbarVisible(entry.target);
            if (isVisible) menuScrollBtn.classList.add("_show"); else menuScrollBtn.classList.remove("_show");
        }
    }
    ...
    function documentActions(event) {
        ...
        if (targetElement.classList.contains("menu__menu-scroll-btn")) checkFinalWidth(-100);
    }
    function checkFinalWidth(num) {
        const initialScrollLeft = menuList.scrollLeft;
        menuList.scrollTo({
            left: initialScrollLeft - num,
            behavior: "smooth"
        });
    }
    const imagesHero = document.querySelectorAll(".images-hero__image");
    for (let index = 0; index < imagesHero.length; index++) {
        const element = imagesHero[index];
        setTimeout((() => {
